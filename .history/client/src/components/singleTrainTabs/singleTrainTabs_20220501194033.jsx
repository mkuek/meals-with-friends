...
const SingleTrainTabs = ({ trainInfo }) => {
    ...
    try {
      if (trainInfo) {
        ...
        const startDate = moment(
          `${trainInfo.meal_date_start.year}-${
            trainInfo.meal_date_start.month
          }-${trainInfo.meal_date_start.day - 1}`,
          "YYYY-M-D"
        );
        const endDate = moment(
          `${trainInfo.meal_date_end.year}-${trainInfo.meal_date_end.month}-${
            trainInfo.meal_date_end.day + 1
          }`,
          "YYYY-M-D"
        );
  ...
  const createEvents = async () => {
    ...
    try {
      ...
      console.log("eventlist", eventListObject);
    } catch (error) {
      dates.map((date, index) => {
        eventListObject.push({
          title: "Available",
                          ...
                          >
                            ...
                            <TabPanel value={innerTabValue} index={0}>
                              <div className="listview">
                                {trainInfo ? renderList() : "No Events"}
                              </div>
                            </TabPanel>
                            <TabPanel
                              value={innerTabValue}
